...
@WebAppConfiguration
class Gh501EnableAuthorizationServerTests {
    ...
    @Test
    void clientAuthenticationFailsThenCustomAuthenticationEntryPointCalled() throws Exception {
        mockMvc.perform(post("/oauth/token").param("grant_type", "client_credentials").header("Authorization", httpBasicCredentials(CLIENT_ID, "invalid-secret"))).andExpect(status().isUnauthorized());
        verify(AuthorizationServerConfig.authenticationEntryPoint).commence(any(HttpServletRequest.class), any(HttpServletResponse.class), any(AuthenticationException.class));
    }
