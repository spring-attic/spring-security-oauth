 ...
 * @author Dave Syer
 */
class ClientCredentialsTokenEndpointFilterTests {
    ...
    @Test
    void testAuthenticationManagerNeeded() {
    ...
    void testAuthentication() throws Exception {
        ...
        filter.setAuthenticationManager(authenticationManager);
        filter.afterPropertiesSet();
        Authentication authentication = new UsernamePasswordAuthenticationToken("foo", "", AuthorityUtils.commaSeparatedStringToAuthorityList("CLIENT"));
        Mockito.when(authenticationManager.authenticate(Mockito.any(Authentication.class))).thenReturn(authentication);
        assertEquals(authentication, filter.attemptAuthentication(request, new MockHttpServletResponse()));
    }
}
