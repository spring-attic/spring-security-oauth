 ...
 * @author efenderbosch
 */
class RedisTokenStoreTests extends TokenStoreBaseTests {
    ...
    @Test
    void testExpiringRefreshToken() throws InterruptedException {
    ...
    @Test
    void testExpiringAccessToken() throws InterruptedException {
    ...
    // gh-572
    @Test
    void storeAccessTokenWithoutRefreshTokenRemoveAccessTokenVerifyTokenRemoved() {
    ...
    // gh-1836
    @Test
    void storeAccessTokenWithRefreshTokenRemoveRefreshTokenAndAccessTokenVerifyTokenRemoved() {
        OAuth2Request request = RequestTokenFactory.createOAuth2Request("clientId", false);
        TestingAuthenticationToken authentication = new TestingAuthenticationToken("user", "password");
        ...
        assertNull(accessTokenValue, "Key refresh_to_access was not deleted!");
        refreshTokenValue = getValue("access_to_refresh:" + oauth2AccessToken.getValue());
        assertNull(refreshTokenValue, "Key access_to_refresh was not deleted!");
        tokenStore.removeAccessToken(oauth2AccessToken);
        Collection<OAuth2AccessToken> oauth2AccessTokens = tokenStore.findTokensByClientId(request.getClientId());
        assertTrue(oauth2AccessTokens.isEmpty());
    }
