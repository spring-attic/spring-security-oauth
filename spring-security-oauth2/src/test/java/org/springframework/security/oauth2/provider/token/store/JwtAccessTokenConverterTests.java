 ...
 * @author Luke Taylor
 */
class JwtAccessTokenConverterTests {
    ...
    void testEnhanceAccessToken() {
        ...
        assertTrue(claims.contains("\"" + AccessTokenConverter.JTI + "\":\"FOO\""), "Wrong claims: " + claims);
        assertTrue(claims.contains("\"" + UserAuthenticationConverter.USERNAME + "\""), "Wrong claims: " + claims);
    }

    @Test
    void testScopePreserved() {
    ...
    @Test
    void testRefreshTokenAdded() throws Exception {
        OAuth2Authentication authentication = new OAuth2Authentication(createOAuth2Request("foo", Collections.singleton("read")), userAuthentication);
        DefaultOAuth2AccessToken original = new DefaultOAuth2AccessToken("FOO");
        original.setScope(authentication.getOAuth2Request().getScope());
    ...
    @Test
    void testExpiringRefreshTokenAdded() throws Exception {
        OAuth2Authentication authentication = new OAuth2Authentication(createOAuth2Request("foo", Collections.singleton("read")), userAuthentication);
        DefaultOAuth2AccessToken original = new DefaultOAuth2AccessToken("FOO");
        original.setScope(authentication.getOAuth2Request().getScope());
    ...
    @Test
    void testRefreshTokenAccessTokenIdWhenDoubleEnhanced() throws Exception {
        OAuth2Authentication authentication = new OAuth2Authentication(createOAuth2Request("foo", Collections.singleton("read")), userAuthentication);
        DefaultOAuth2AccessToken original = new DefaultOAuth2AccessToken("FOO");
        original.setScope(authentication.getOAuth2Request().getScope());
        original.setRefreshToken(new DefaultOAuth2RefreshToken("BAR"));
    ...
    @Test
    void rsaKeyCreatesValidRsaSignedTokens() throws Exception {
    ...
    @Test
    void publicKeyStringIsReturnedFromTokenKeyEndpoint() throws Exception {
    ...
    @Test
    void publicKeyStringIsReturnedFromTokenKeyEndpointWithNullPrincipal() throws Exception {
        tokenEnhancer.setVerifierKey("-----BEGIN RSA PUBLIC KEY-----\n" + "MGgCYQDk3m+AGfjcDrT4fspyIBqmulFjVXuiciYvpaD5j2XaR7c6Krm5wsBLOiUo\n" + "kmd6wbrRAMPMpoC1eogWNNoXY7Jd4eWdDVmscfHczGX13uBKXwdOCEqKqoWQsXIb\n" + "7kgz+HkCAwEAAQ==\n" + "-----END RSA PUBLIC KEY-----");
        Map<String, String> key = tokenEnhancer.getKey();
        assertTrue(key.get("value").contains("-----BEGIN"), "Wrong key: " + key);
    }
    ...
    @Test
    void sharedSecretIsReturnedFromTokenKeyEndpoint() throws Exception {
        tokenEnhancer.setVerifierKey("someKey");
        assertEquals("{alg=HMACSHA256, value=someKey}", tokenEnhancer.getKey().toString());
    }

    @Test
    void keysNotMatchingWithMacSigner() throws Exception {
    ...
    @Test
    void publicKeyOnlyAllowedForVerification() throws Exception {
        tokenEnhancer.setVerifierKey("-----BEGIN RSA PUBLIC KEY-----\n" + "MGgCYQDk3m+AGfjcDrT4fspyIBqmulFjVXuiciYvpaD5j2XaR7c6Krm5wsBLOiUo\n" + "kmd6wbrRAMPMpoC1eogWNNoXY7Jd4eWdDVmscfHczGX13uBKXwdOCEqKqoWQsXIb\n" + "7kgz+HkCAwEAAQ==\n" + "-----END RSA PUBLIC KEY-----");
        tokenEnhancer.afterPropertiesSet();
    ...
    // gh-1111
    @Test
    void setJwtClaimsSetVerifierWhenNullValueThenThrowIllegalArgumentException() throws Exception {
    ...
    // gh-1111
    @Test
    void decodeWhenJwtClaimsSetVerifierIsSetThenVerifyIsCalled() throws Exception {
    ...
    private OAuth2Request createOAuth2Request(String clientId, Set<String> scope) {
        return new OAuth2Request(Collections.<String, String>emptyMap(), clientId, null, true, scope, null, null, null, null);
    }
