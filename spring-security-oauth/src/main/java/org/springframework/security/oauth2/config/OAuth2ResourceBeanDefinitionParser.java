...
import org.springframework.security.oauth2.consumer.webserver.WebServerFlowResourceDetails;
...
public class OAuth2ResourceBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
  ...
  protected Class getBeanClass(Element element) {
    if ("web-server".equals(element.getAttribute("type")) || "web_server".equals(element.getAttribute("type"))) {
      return WebServerFlowResourceDetails.class;
    }
  ...
  protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
    ...
    if (!StringUtils.hasText(type)) {
      parserContext.getReaderContext().error("An type must be supplied on a resource element.", element);
    }
    builder.addPropertyValue("flowType", type);
