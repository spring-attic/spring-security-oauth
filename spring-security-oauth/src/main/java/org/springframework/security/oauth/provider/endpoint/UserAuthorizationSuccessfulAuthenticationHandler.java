...
import static org.springframework.security.oauth.provider.endpoint.AuthorizationEndpoint.CALLBACK_ATTRIBUTE;
import static org.springframework.security.oauth.provider.endpoint.AuthorizationEndpoint.VERIFIER_ATTRIBUTE;
 ...
 * success URL. Otherwise, the oauth_verifier and oauth_token parameters are appended to the callback URL and the user
 * is redirected.
 *
...
public class UserAuthorizationSuccessfulAuthenticationHandler extends SimpleUrlAuthenticationSuccessHandler {
	...
	private static final Log LOG = LogFactory.getLog(UserAuthorizationSuccessfulAuthenticationHandler.class);
	 ...
	 */
	public UserAuthorizationSuccessfulAuthenticationHandler(String defaultTargetUrl) {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
		...
		if (callbackURL == null) {
			if (!isRequire10a()) {
				...
				throw new IllegalStateException("Callback URL was not loaded into the request. Endpoint never called?");
			}
		}
